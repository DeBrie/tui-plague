import { Country, GameState, NewsItem, NewsType, Plague } from "../types.js";

// Track what news we've already generated to avoid duplicates
interface NewsTracker {
  firstCaseReported: boolean;
  countriesInfected: Set<string>;
  countriesWithDeaths: Set<string>;
  countriesClosed: Set<string>;
  airportsClosed: Set<string>;
  seaportsClosed: Set<string>;
  cureStarted: boolean;
  cureMilestones: Set<number>;
  awarenessAlerts: Set<string>;
  devastatedCountries: Set<string>;
}

let newsTracker: NewsTracker = {
...
const firstCaseHeadlines = (country: string, plague: string): string[] => [
...
const spreadHeadlines = (country: string, plague: string): string[] => [
...
const deathHeadlines = (country: string, plague: string): string[] => [
...
const massDeathHeadlines = (country: string, deaths: string): string[] => [
...
const borderClosedHeadlines = (country: string): string[] => [
...
const airportClosedHeadlines = (country: string): string[] => [
...
const seaportClosedHeadlines = (country: string): string[] => [
...
const cureStartedHeadlines = (plague: string): string[] => [
...
const cureProgressHeadlines = (plague: string, progress: number): string[] => [
...
const awarenessHeadlines = (country: string, plague: string): string[] => [
...
const devastatedHeadlines = (country: string): string[] => [
...
const generalHeadlines = (plague: string): string[] => [
...
const symptomHeadlines = (symptom: string, plague: string): string[] => [
...
export const generateNews = (
  ...
  if (!newsTracker.firstCaseReported && newState.totalInfected > 0) {
    const startingCountry = countries.find(
      (c) => c.id === newState.startingCountry
    );
    if (startingCountry) {
      news.push(
        createNewsItem(
          day,
          "first_case",
          pickRandom(firstCaseHeadlines(startingCountry.name, plagueName)),
          10
        )
      );
  ...
  for (const country of countries) {
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "spread_to_country",
          pickRandom(spreadHeadlines(country.name, plagueName)),
          8
        )
      );
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "deaths_start",
          pickRandom(deathHeadlines(country.name, plagueName)),
          9
        )
      );
    ...
    const deathPercent =
      Math.floor((country.dead / country.population) * 10) * 10;
    if (
      deathPercent >= 10 &&
      ...
      if (
        ...
        news.push(
          createNewsItem(
            day,
            "country_devastated",
            pickRandom(devastatedHeadlines(country.name)),
            10
          )
        );
        ...
        news.push(
          createNewsItem(
            ...
            "mass_deaths",
            pickRandom(
              massDeathHeadlines(country.name, formatNumber(country.dead))
            ),
            7
          )
        );
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "border_closed",
          pickRandom(borderClosedHeadlines(country.name)),
          7
        )
      );
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "airport_closed",
          pickRandom(airportClosedHeadlines(country.name)),
          6
        )
      );
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "seaport_closed",
          pickRandom(seaportClosedHeadlines(country.name)),
          6
        )
      );
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "awareness",
          pickRandom(awarenessHeadlines(country.name, plagueName)),
          5
        )
      );
  ...
  if (!newsTracker.cureStarted && cureProgress > 0) {
    news.push(
      createNewsItem(
        day,
        "cure_started",
        pickRandom(cureStartedHeadlines(plagueName)),
        9
      )
    );
    ...
    if (
      ...
      news.push(
        createNewsItem(
          day,
          "cure_progress",
          pickRandom(cureProgressHeadlines(plagueName, milestone)),
          8
        )
      );
  ...
  if (Math.random() < 0.02 && visibility > 20) {
    news.push(
      createNewsItem(
        day,
        "general",
        pickRandom(generalHeadlines(plagueName)),
        2
      )
    );
  }
